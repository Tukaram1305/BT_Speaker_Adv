...
{
 public:
  diamond(uint16_t centerX, uint16_t centerY, uint16_t radius, int dph) : x(centerX), y(centerY), rad(radius), depth(dph)
  { 
    // prawo -> lewo      dol->gora->dol
    movX=rad;  movY=0;
    diam.createSprite(rad*2, rad*2);
    ctY = rad;
    ctX = rad;
    maxIter = rad*2;
    //rotPerIter = 1.5707963267948966192313216916398/(float(maxIter)); dla 90
    rotPerIter = 3.1415/(float(maxIter));  // dla 180
    ...
    // calc start pos
      ...
      sqEx=x;       sqEy=y+rad;   
      sqBx=x+rad/2; sqBy=y;
      sqAx=x+rad/2; sqAy=y+rad;   
      sqDx=x+rad;   sqDy=y;
      sqFx=x+rad;   sqFy=y+rad;

      vmovABx = rad; vmovAy=0; vmovBy=0; vmovABx_p = rad; vmovAy_p = 0; vmovBy_p=0;
      xPandCDEF = 0; xPandCDEF_p = 0;
  }
  ~diamond(){}

  void run()
  {
    if (rotSpd.del(calcSpd))
    {
      // L-Top
      diam.fillTriangle(ctX-rad, ctY, ctX, ctY-rad, ctX+movX, ctY+movY, hsv2hex(1+hue,180,shading) );
      // R-Top
      diam.fillTriangle(ctX+rad, ctY, ctX, ctY-rad, ctX+movX, ctY+movY, hsv2hex(1+hue,150,lihting) );
      // L-Bot
      diam.fillTriangle(ctX-rad, ctY, ctX, ctY+rad, ctX+movX, ctY+movY, hsv2hex(45+hue,180,shading) );
      // R-Bot
      diam.fillTriangle(ctX+rad, ctY, ctX, ctY+rad, ctX+movX, ctY+movY, hsv2hex(45+hue,150,lihting) );
      // enges
      // L-Top
      diam.drawTriangle(ctX-rad, ctY, ctX, ctY-rad, ctX+movX, ctY+movY, TFT_WHITE );
      // R-Top
      diam.drawTriangle(ctX+rad, ctY, ctX, ctY-rad, ctX+movX, ctY+movY, TFT_WHITE );
      // L-Bot
      diam.drawTriangle(ctX-rad, ctY, ctX, ctY+rad, ctX+movX, ctY+movY, TFT_WHITE );
      // R-Bot
      diam.drawTriangle(ctX+rad, ctY, ctX, ctY+rad, ctX+movX, ctY+movY, TFT_WHITE );
      diam.pushSprite(x-rad, y-rad);

      //calc movX
      if (movX > -rad)  movX--;  
      if (movX <= -rad) { movX = rad; movY=0; lihting=90; shading=240; curRot=0; hue++; }
      if (movX%3==0 && movX>=0) movY--;
      if (movX%3==0 && movX<0 ) movY++;
      
      if (lihting < 254) lihting+=2;
      if (shading > 90) shading-=2;
      curRot+=rotPerIter;
      calcSpd = static_cast<int>(15+(sin(curRot)*10));
      
      //calc movY
      //float nHypo = float(abs(movX))/cose;
      //movY = round(nHypo*sine);
    }
  }
  ...
  void runSqr()
  {
    if (rotSpd.del(calcSpd))
    {
       ...
       */
       ...
       // czysc
      // 1 = a c e
      tft.fillTriangle(sqAx+vmovABx_p,sqAy+vmovAy_p,  sqCx-xPandCDEF_p,sqCy,  sqEx-xPandCDEF_p,sqEy,  TFT_BLACK );
      // 2 = a b c
      tft.fillTriangle(sqAx+vmovABx_p,sqAy+vmovAy_p,  sqBx+vmovABx_p,sqBy-vmovBy_p,   sqCx-xPandCDEF_p,sqCy,  TFT_BLACK );
      // 3 = a d b
      tft.fillTriangle(sqAx+vmovABx_p,sqAy+vmovAy_p,  sqDx+xPandCDEF_p,sqDy,  sqBx+vmovABx_p,sqBy-vmovBy_p,  TFT_BLACK );
      // 4 = a f d
      tft.fillTriangle(sqAx+vmovABx_p,sqAy+vmovAy_p,  sqFx+xPandCDEF_p,sqFy,  sqDx+xPandCDEF_p,sqDy,  TFT_BLACK );
      //diam.pushSprite(x-rad, y-rad);
      
      // 1 = a c e
      tft.fillTriangle(sqAx+vmovABx,sqAy+vmovAy,  sqCx-xPandCDEF,sqCy,  sqEx-xPandCDEF,sqEy,  hsv2hex(15+hue,180,lihting) );
      // 2 = a b c
      tft.fillTriangle(sqAx+vmovABx,sqAy+vmovAy,  sqBx+vmovABx,sqBy-vmovBy,   sqCx-xPandCDEF,sqCy,  hsv2hex(15+hue,180,lihting) );
      // 3 = a d b
      tft.fillTriangle(sqAx+vmovABx,sqAy+vmovAy,  sqDx+xPandCDEF,sqDy,  sqBx+vmovABx,sqBy-vmovBy,  hsv2hex(1+hue,180,shading) );
      // 4 = a f d
      tft.fillTriangle(sqAx+vmovABx,sqAy+vmovAy,  sqFx+xPandCDEF,sqFy,  sqDx+xPandCDEF,sqDy,  hsv2hex(1+hue,180,shading) );
      //diam.pushSprite(x-rad, y-rad);
      
      //calc movement
      vmovABx_p = vmovABx; vmovAy_p = vmovAy; vmovBy_p=vmovBy; xPandCDEF_p = xPandCDEF;
      if (vmovABx> -rad/2) { vmovABx-=1; }
      if (vmovABx<= -rad/2) { vmovABx=rad/2; vmovAy=0; vmovBy=0; lihting=90; shading=240; curRot=0; } // RESET

      if (vmovABx%4==0 && vmovABx>0) {vmovAy+=1; vmovBy+=1; xPandCDEF++; }
      if (vmovABx%4==0 && vmovABx<0) {vmovAy-=1; vmovBy-=1; xPandCDEF--; }
      hue++; 
      if (lihting < 250) lihting+=6;
      if (shading > 120) shading-=6;
      curRot+=rotPerIter;
      calcSpd = static_cast<int>(20+(sin(curRot)*15));
    }
  }
  
  private:
  int sqAx,sqAy, sqBx,sqBy, sqCx,sqCy, sqDx,sqDy, sqEx,sqEy, sqFx,sqFy;
  int vmovABx, vmovAy, vmovBy, xPandCDEF;
  int vmovABx_p, vmovAy_p, vmovBy_p, xPandCDEF_p;
  float curRot;
  //const float deg90rad = 1.5707963267948966192313216916398;
  float rotPerIter;
  uint16_t maxIter;
  int calcSpd;
  byte shading = 230;
...
class valDisplay
{
  ...
  void draw(const char* prefix, const char* afix, int val, int x, int y, int w, int h)
  {
    ...
    */
    ...
    // sprite
    sprTxt.createSprite(w, h);
    sprTxt.fillRect(0, 0, w, h, TFT_BLACK);
    sprTxt.drawRect(0, 0, w, h, TFT_BLUE);
    String form = prefix+String(val)+afix;
    sprTxt.setTextColor(TFT_WHITE);
    sprTxt.drawCentreString(form, w/2, h/2-8, 2);
    sprTxt.pushSprite(x, y);
    sprTxt.deleteSprite();
  }
};
...
class valBarDisp
{
  ...
  void draw(int val, int MIN, int MAX, int x, int y, int w, int h)
  {
    ...
    */
    // podejscie sprite - cala belka ~ 190x15 x 2(16B) = 5700B = 5,5664kB
    spr.createSprite(w, h);
    //spr.fillSprite(TFT_BLACK);
    spr.fillRect(0, 0, w, h, TFT_BLACK);
    spr.drawRect(0, 0, w, h, TFT_BLUE);
    float DELTA = MAX-MIN;
    float coef = float(w/DELTA);
    float colCoef = float(100/DELTA);
    int BARCOL = int(colCoef*val);
    int BAR = round(coef*val);
    spr.fillRect(1, 1, BAR-2, h-2, hsv2hex(100-BARCOL, 180, 250));
    //podzialki
    int DIV = w/5;
    for (int i=1; i<DIV-1; i++)
    {
      spr.drawFastVLine( i*5, 1, h-2 ,TFT_BLACK );
      spr.drawFastVLine( 1+i*5, 1, h-2 ,TFT_BLACK );
    }
    spr.pushSprite(x, y);
    spr.deleteSprite();
  }
...
class spkLedSelector
{
  ...
  void draw_RGB(uint16_t rgb_SX, uint16_t rgb_SY, uint16_t rgb_W, uint16_t rgb_H)
  {
    ...
    tft.drawCentreString("off", rgb_sx+rgb_w/2, rgb_sy+12, 1);
  }
...
//tft.drawRect(cx - i2, cy - i2, i, i, color);
  ...
  bool chkIfPressHSV(uint16_t X, uint16_t Y)
  {
    ...
    {
    tft.fillRect(rgb_sx, rgb_sy, rgb_w, rgb_h, TFT_BLACK );
    tft.drawRect(rgb_sx-1, rgb_sy-1, rgb_w+2, rgb_h+2, TFT_BLACK);
    tft.drawRect(rgb_sx-2, rgb_sy-2, rgb_w+4, rgb_h+4, TFT_WHITE);
    tft.drawCentreString("off", rgb_sx+rgb_w/2, rgb_sy+12, 1);
    // Wylacz cale podswietlenie
    BGcol.v = 0; mRing.v = 0; LeftL.v = 0; LeftR.v = 0; RightL.v = 0; RightR.v = 0;
    STATIC_LIGHTS = true;
      return true;
    }// Dotyk na RGB
...
{
  public:
  oscylogram(/*x = 0*/ uint16_t win_y, /*w = 320px*/ uint16_t win_h)
  {
    this->y = win_y;
    this->h = win_h;
    this->x = 0;
  ...
  void start()
  {
    ...
    for (int i=0; i<40; i++) { arr[i] = 0; arrp[i]=0; }
  }
  ...
  void grid()
  {
    if (h>0)
    {
      ...
      for (int i=1; i<6; i++)
      {
        tft.drawFastHLine(2, y+midH*i, 316, 0x4208 );
      }
      int midV = floor(320/11);
      for (int i=1; i<13; i++)
      {
        tft.drawFastVLine(midV*i, y+2, h-4, 0x4208 );
      } 
    }
  }
  ...
  {
    adc=0;
    adc = static_cast<int>(analogRead(MIC) - MICOFF);
    if (adc > -NOISERED && adc < NOISERED) { adc = 0; }
    
    if (adc > AMAX) arr[0] = h/2-2;
    else if (adc < -AMAX) arr[0] = -(h/2)+2;
    else arr[0] = map(adc, -AMAX, AMAX, -(h/2)+2, h/2-2);
    if (arr[0] < NOISERED && arr[0] > -NOISERED) arr[0] = 0;
    //int COEF = 320/40;
    const int COEF = 8;
    // czyszczenie
    for (int i=0; i<38; i++)
    {
      tft.drawLine((i*COEF)+2, y+(h/2)+arrp[i], ((i+1)*COEF)+2, y+(h/2)+arrp[i+1], TFT_BLACK);
    }
    // rysowanie
    this->grid();
    for (int i=0; i<38; i++)
    {
      tft.drawLine((i*COEF)+2, y+(h/2)+arr[i], ((i+1)*COEF)+2, y+(h/2)+arr[i+1], TFT_RED);
    }
    // backup tablicy dla czyszczenia
    for (int i=0; i<40; i++) arrp[i] = arr[i];
    // przestawienie
    for (int i=38; i>=0; i--) arr[i+1] = arr[i];
    //delay(5);
  }
