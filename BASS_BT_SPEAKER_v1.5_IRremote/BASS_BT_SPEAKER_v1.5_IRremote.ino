 ...
 * Moje klasy
 * class Kronos             (void)
 * class JSONcreator        (void)
 * class button             (const char* nazwa, int delay)
 * class valDisplay         (void)
 * class valBarDisp         (void)
 * class selectorFrame      (void)
 * class toggler            (const char* nazwa, bool state)
 * class radioBtn           (const char* nazwa)
 * class ValElem            ([i/u/f/d/w/b] *val, f/i vmin, f/i vmax, f/i INC, const char *NAZWA)
 * class ValChngObjArrtList (ValElem** tab, byte elem_num)
 * class spkLedSelector     (void)
 * class oscylogram         (uint16_t window_y, uint16_t window_height)
 */
...
TFT_eSprite diam = TFT_eSprite(&tft);
...
myLed BGcol = (myLed) {.h=122, .s=0, .v=0 };
...
diamond dmfx(230, 150, 30, 8);
//diamond dmfx(215, 135, 30, 8);
void drawConfSite()
{
  tft.fillScreen(TFT_BLACK);
  ...
  if (WiFi.status() == WL_CONNECTED)
  {tft.drawCentreString(WiFi.localIP().toString(), 230, 150-48, 2);}
  else{tft.drawCentreString("OFFLINE", 230, 150-48, 2);}
}
