...
// STRONY       // glowna(vol)      ustawienia        podswietlenie
button        SITE_MAIN("MAIN",500),    SITE_CONF("CONF",500),    SITE_LED("BLED",500);
...
//uint16_t CHNG_SITE_AT = 800;    // opoznienie dotyku po zmianie strony
//unsigned long CHNG_SITE_AT_TIMER = 0; // tylko timer dla dotyku
...
void chkIfPotsChange()
{
  ...
  { 
    ...
    SUB_VALDISP.draw("Bass: ", " %", M_BASAMP , 65, 55, 190, 15);
  ...
  { 
    ...
    SPK_VALDISP.draw("LSpk/RSpk: ", " %", M_SPKAMP , 65, 100, 190, 15);
...
{
  tft.fillScreen(TFT_BLACK);
  LEDFX_SELECTOR.draw(10,110,120,30);
  SITE_MAIN.draw(240, 180, 80, 45);
  SITE_LED.draw(140, 180, 80, 45);
  SETTINGS_MENU.draw(10, 10, 210);
...
void drawLedSite()
{
  ...
  virtualSpk.draw_RGB(160, 205, 50, 30);
...
if (siteSelector==1 && SITE_DRAWN==false) { drawConfSite(); SITE_DRAWN=true; itemSelector=0; }
if (siteSelector==2 && SITE_DRAWN==false) { drawLedSite();  SITE_DRAWN=true; itemSelector=0; }
